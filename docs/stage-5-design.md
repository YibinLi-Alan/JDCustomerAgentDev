...
- 判据写进 prompt:**无需查询/操作数据(自报信息、寒暄)→ "direct" 客服直接回复;
  单域可答 → 直派对应专员;跨域/多动作/需先查证再操作 → "supervisor"**;
- 解析失败/未知目标 → 降级派 supervisor(宁可走贵的全能路径,不给用户错误专员);
- 这是 Swarm handoff 的入口版:分诊只发生一次,派出后该专员全权负责本轮。

> 修订(2026-07-09 用户自测后评审新增):增加 **"direct" 第三出口**。自测发现
> 「自报姓名地址」被升级 supervisor 并建了不该建的工单——告知类输入原本没有出路,
> 只能被硬塞进带工具的派工路径。direct 出口 = 不派工,由轻量客服 prompt(无工具)
> 一次调用直接回复,是三级分流里最便宜的一级(direct < 专员直派 < supervisor)。
