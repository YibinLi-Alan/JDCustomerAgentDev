...
- **默认权重:`w_rel=1.0, w_rec=0.25, w_imp=0.25`——相关性主导**,时近性与重要性
  只当加分项/平局裁决,防止「重要性 10 但与当前问题无关」的记忆(如投诉史)挤掉真正相关的记忆。
  评审初定 0.5/0.5,后由评测⑥实测修正:0.25/0.25 的 Hit@1 94% / Recall@3 100%,
  优于 0.5/0.5(81%/94%)、纯相关性(81%/100%,输在新旧地址陷阱)与论文平权
  (38%/75%,高重要记忆到处插队)—— 数据驱动改默认,详见 `docs/stage-4-memory-eval.md`
