...
## 8. 工具清单与编写规范(评审后定稿,2026-07-06 与导师共同确认)

### 8.1 工具编写规范(七条,每个工具必须遵守)

1. **description 三段式**:`一句话功能。何时用:XXX。参数:XXX。`
2. **命名**:`动词_名词` 的 snake_case;
3. **strict mode 默认开**,所有工具不例外;
4. **「查无结果」不算错误**:订单不存在 / FAQ 没命中返回正常提示文本(`ok=True`),
   让模型转述;只有系统性故障(超时、参数非法)才走 `ok=False`;
5. **mock 数据统一收口**:共享数据层 `tools/jd_mock_data.py`(`JDMockStore`),
   订单 ↔ 运单 ↔ 商品 ↔ 退款互相一致;接真 API = 换数据层,工具零改动;
6. **每个工具的测试**:正常路径 ≥1 条 + 异常/查无路径 ≥1 条;
7. **权限标记进框架(评审决定)**:`BaseTool.permission: low | medium | high`,
   本阶段只声明不拦截,阶段六 safety 读它接分层执行 + HITL。

### 8.2 最终清单(11 个 = 原 8 个 + 评审追加 3 个,全部已实现)

**A 组 · 业务工具(JD mock,共享 `JDMockStore`)**

| 工具 | 何时用 | 权限 |
|---|---|---|
| `query_order` | 用户问某订单进度/状态/是否发货 | 低 |
| `query_logistics` | 已有运单号,查包裹位置/预计送达 | 低 |
| `query_product` | 用户问商品价格/库存/是否有货/保修 | 低 |
| `search_faq` | 用户问平台规则(退换货政策、发票、优惠券);查无时引导转人工 | 低 |
| `query_user_orders` **(追加)** | 用户没报订单号(“我最近买的东西”),先列订单再查详情 | 低 |
| `apply_refund` | 用户明确要退款且已确认订单号与理由;未发货引导改走取消 | **高(HITL)** |
| `cancel_order` **(追加)** | 取消**未发货**订单;已发货拒绝并引导退款(状态机) | **高(HITL)** |
| `create_ticket` **(追加)** | 问题超出工具能力/用户点名要人工 → 建人工工单 | **高(HITL)** |

**B 组 · 通用工具(对齐大纲示例)**

| 工具 | 何时用 | 权限 |
|---|---|---|
| `calculator` | 需要精确算金额:差价、折扣、退款分摊(AST 白名单求值,不用 eval) | 低 |
| `current_time` | 需要“今天/现在”才能答:预计送达还有几天、是否超退货期 | 低 |
| `http_request` | 需要调外部 HTTP API 取数(域名白名单 + 限时 + 限响应体) | 中 |

装配入口:`tools/presets.py::default_registry()` 一行拿到 11 件套。
大纲 ≥5 的要求超额达成(11 > 5)。
