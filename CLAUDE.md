...
> **Progress: Stages 1 ✅ 2 ✅ 3 ✅ 4 ✅ are done (as of 2026-07-07).** Built so far: `core/` (LLM interface w/ native function calling `chat(tools=...)` + `ToolCall` + Claude/OpenAI providers + `create_llm` factory + config), two agent loops in `core/agent.py` (`ReActAgent` text-parsing fallback + `ToolCallingAgent` native function calling w/ parallel calls), the tool system (`tools/`: `BaseTool` w/ strict mode + timeout + `permission` tiers, `ToolResult`, `ToolRegistry`, `@tool` decorator, shared `JDMockStore` data layer, 11 tools — 8 JD business + 3 common — assembled via `presets.default_registry()`), the memory system (`memory/`: `ShortTermMemory` sliding window w/ token budget + turn atomicity, `SummaryCompressor` recursive summarization w/ fallback, `LongTermMemory` — LLM fact extraction → Mem0-style ADD/UPDATE/DELETE/NOOP reconcile → Generative-Agents 3-factor retrieval (weights 1.0/0.25/0.25, tuned by eval) — over `Embedder`/`VectorStore` protocols (OpenAI embeddings; Chroma persistent + in-memory stores; user_id isolation enforced at storage layer), unified `MemoryManager` facade + `create_memory_manager()` factory, retrieval-accuracy eval in `evaluation/`), four CLIs (`examples/chat_cli.py`, `react_cli.py`, `tools_cli.py`, `memory_cli.py` w/ multi-user + cross-session demo), and offline tests (`tests/`, 113 passing, MockLLM/MockEmbedder). Stages 5–6 remain. Build strictly according to the curriculum below; do not invent structure that contradicts it, and do not narrow the scope to a single hard-coded business script.
