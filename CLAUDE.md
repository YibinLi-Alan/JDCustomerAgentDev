...
> **Progress: Stages 1 ✅ 2 ✅ 3 ✅ 4 ✅ 5 ✅ are done (as of 2026-07-09).** Built so far: `core/` (LLM interface w/ native function calling `chat(tools=...)` + `ToolCall` + Claude/OpenAI providers + `create_llm` factory + config), two agent loops in `core/agent.py` (`ReActAgent` text-parsing fallback + `ToolCallingAgent` native function calling w/ parallel calls), the tool system (`tools/`: `BaseTool` w/ strict mode + timeout + `permission` tiers, `ToolResult`, `ToolRegistry` w/ `subset()`, `@tool` decorator, shared `JDMockStore` data layer, 11 tools incl. a 7-day refund-window rule on `apply_refund`, assembled via `presets.default_registry()`), the memory system (`memory/`: `ShortTermMemory` sliding window w/ token budget + turn atomicity, `SummaryCompressor` recursive summarization w/ fallback, `LongTermMemory` — LLM fact extraction → Mem0-style ADD/UPDATE/DELETE/NOOP reconcile → Generative-Agents 3-factor retrieval (weights 1.0/0.25/0.25, tuned by eval) — over `Embedder`/`VectorStore` protocols, unified `MemoryManager` facade, retrieval eval in `evaluation/`), planning & multi-agent (`planning/`: `Planner` linear plans w/ specialist assignment + fallback degradation + `replan()`; `PlanExecutor` sequential dispatch via `StepRunner` protocol + `ScratchPad` shared blackboard + ≤1 replan; `multi_agent/`: `Specialist` wrapper + `TaskAssignment`/`TaskOutcome` + "无法完成:" failure-marker protocol, 3 business specialists (order/aftersales/product, tool subsets), `Router` triage (degrades to supervisor), `Supervisor` central dispatch (plan-driven, not LLM-free-choice), `Critic` final-answer review w/ ≤1 rewrite; core/agent.py untouched), five CLIs (`examples/chat_cli.py`, `react_cli.py`, `tools_cli.py`, `memory_cli.py`, `multi_agent_cli.py` w/ /mode + /trace), and offline tests (`tests/`, 161 passing). Stage 6 remains. Build strictly according to the curriculum below; do not invent structure that contradicts it, and do not narrow the scope to a single hard-coded business script.
