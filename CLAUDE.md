...
> **Progress: Stages 1 ✅ 2 ✅ 3 ✅ are done (as of 2026-07-06).** Built so far: `core/` (LLM interface w/ native function calling `chat(tools=...)` + `ToolCall` + Claude/OpenAI providers + `create_llm` factory + config), two agent loops in `core/agent.py` (`ReActAgent` text-parsing fallback + `ToolCallingAgent` native function calling w/ parallel calls), the tool system (`tools/`: `BaseTool` w/ strict mode + timeout + `permission` tiers, `ToolResult`, `ToolRegistry`, `@tool` decorator, shared `JDMockStore` data layer, 11 tools — 8 JD business + 3 common — assembled via `presets.default_registry()`), three CLIs (`examples/chat_cli.py`, `react_cli.py`, `tools_cli.py`), and offline tests (`tests/`, 70 passing, MockLLM). Stages 4–6 remain. Build strictly according to the curriculum below; do not invent structure that contradicts it, and do not narrow the scope to a single hard-coded business script.
