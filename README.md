...
<p align="left">
  ...
  <img alt="tests" src="https://img.shields.io/badge/tests-226%20passing-brightgreen">
...
- ✅ **质量可量化**:226 条离线单测(零 API 成本)+ LLM-as-Judge 端到端评测(76% 通过)+
...
> **`examples/`、`tests/`、`docs/` 都在仓库根目录**(与 `agent_framework/` 平级),
> **不在 `agent_framework/` 里面**——框架核心 `agent_framework/` 刻意保持「纯库」,
> 不含可执行脚本与测试。跑 demo 用 `python -m examples.xxx`、跑测试用 `pytest tests/`。

```
JD/                          # 仓库根
├── agent_framework/         # ← 框架核心(纯库,9 个子包)
...
├── examples/                # ← 8 个 CLI 演示(python -m examples.xxx)
├── tests/                   # ← 226 条离线单测(pytest tests/,MockLLM/MockEmbedder)
└── docs/                    # ← 设计文档 / 架构图 / 设计论文 / 测试手册 / 评测报告 / 部署
```
...
```bash
python -m pytest tests/ -q      # 226 条离线单测(MockLLM,零 API 成本)
ruff check .                    # 代码规范
black --check .                 # 格式
...
| 文档 | 内容 |
|---|---|
| [`docs/项目综述.md`](docs/项目综述.md) | **High-Level 综述**:业务/实现方式/工具/论文依据/量化结果/功能/局限(答辩·简历·汇报用) |
| [`docs/技术点精讲.md`](docs/技术点精讲.md) | **简明版**:28 个技术点,每个讲清「是什么/优点/怎么实现」,专业名词当场解释 |
| [`docs/设计论文.md`](docs/设计论文.md) | 完整版:六阶段每个问题的思考/设计/依据论文,逐条回答培训大纲 |
